...
function SimpleVideo({ placeholder, styles, brightness, priority = false, title = "Video" }) {
    ...
    useEffect(() => {
        ...
        // Force initial scale calculation
        setTimeout(updateScale, 100);

        // Also update scale when container becomes visible
    ...
    return (
        ...
        >
            ...
            <div
                className={`absolute top-0 left-0 w-full h-full min-h-[80vh] z-[1] brightness-[100%] ${styles || brightness}`}
            ></div>
        </div>
    );
}
